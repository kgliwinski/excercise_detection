...
typedef enum { BOSCH_ACCELEROMETER_ONLY, BOSCH_MAGNETOMETER_ONLY, BOSCH_ACCEL_AND_MAGN } CfgBoshSensor_t;
...
class ContinuousMode {
 public:
  ...
  void clearAccelerationAvailable() { _availableA = 0; }
  void clearGyroscopeAvailable() { _availableG = 0; }
  const size_t getSamplesCount() const { return samples_count; }

  bmi2_sens_axes_data* getAccelData() { return accel_data; }
  bmi2_sens_axes_data* getGyroData() { return gyro_data; }

  operator bool() const { return continuousMode == true; }
...
class BoschSensorClass {
 public:
  ...
  void setInterruptPin(PinName irq_pin) { BMI270_INT1 = irq_pin; }
  void setInterruptPin(pin_size_t irq_pin) { BMI270_INT1 = digitalPinToPinName(irq_pin); }
  PinName BMI270_INT1 = NC;
#endif
#ifdef ARDUINO_ARCH_ESP32
  void onInterrupt(void (*)(void));
  void setInterruptPin(int irq_pin) { BMI270_INT1 = irq_pin; }
  ...
  ContinuousMode& getContinuousMode() { return continuousMode; }
